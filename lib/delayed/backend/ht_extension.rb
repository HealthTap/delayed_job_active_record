    ...
    module HtExtension
      ...
      def _payload_object
        if self.handler.present?
          ...
          raise ArgumentError.new("object not found for delayed job #{self.id}, type: #{self.delayed_object_type}, obj_id: #{self.delayed_object_id}, method: #{self.method_name}") unless object.present?
